...
#### 方式
- **L1范数剪枝**：基于权重的L1范数大小，移除绝对值较小的权重
- **结构化剪枝**：移除整个神经元或通道，保持模型结构
- **非结构化剪枝**：随机移除单个权重，可能破坏模型结构

#### 使用场景
- **边缘设备部署**：如手机、IoT设备等资源受限环境
- **实时推理需求**：需要低延迟响应的应用场景
- **模型压缩**：减少模型存储和传输成本
...
#### 方式
- **动态量化**：仅量化权重，推理时动态计算激活值
- **静态量化**：同时量化权重和激活值，需要校准数据
- **训练后量化**：在训练完成后进行量化，实现简单
- **量化感知训练**：在训练过程中考虑量化影响，获得更好的精度

#### 使用场景
- **移动设备部署**：如手机、平板等资源受限设备
- **嵌入式系统**：如智能摄像头、工业控制系统
- **云端推理加速**：提高服务器端推理吞吐量
...
#### 方式
- **基于软标签的蒸馏**：使用教师模型的输出概率分布作为软标签
- **基于特征的蒸馏**：使用教师模型中间层的特征作为监督信号
- **多教师蒸馏**：使用多个教师模型共同指导学生模型

#### 使用场景
- **资源受限环境**：需要小模型但又希望保持较好性能的场景
- **模型压缩**：在显著减小模型规模的同时保持性能
- **知识迁移**：将复杂模型的知识迁移到简单模型

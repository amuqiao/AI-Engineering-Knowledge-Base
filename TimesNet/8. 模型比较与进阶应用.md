...
flowchart LR
    ...
    %% 核心说明
    Note["TimesNet 与其他模型比较：

预测精度：TimesNet > Autoformer > Informer > Transformer > LSTM/GRU

计算效率：TimesNet ≈ LSTM/GRU > Autoformer > Informer > Transformer

内存消耗：TimesNet ≈ LSTM/GRU < Autoformer < Informer < Transformer

适用序列长度：TimesNet (长) > Informer/Autoformer (中等) > Transformer (短) > LSTM/GRU (短)

最佳应用场景：
- TimesNet：长时序预测、周期性强的序列
- Autoformer：周期性强的序列
- Informer：中等长度时序
- Transformer：中等长度时序、需要并行计算
- LSTM/GRU：短时序、实现简单"]:::coreNoteStyle
